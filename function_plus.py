...
class Cols:
    ...
    def __init__(self):
        ...
        self.handlers = {
            ...
            3: lambda: self.halfptr().format(idc.get_func_attr(
                self.addr, idc.FUNCATTR_END) - self.addr),
            4: lambda: self.set_if_true(idc.get_func_attr(
                self.addr, idc.FUNCATTR_FRSIZE)),
            5: lambda: self.set_if_true(idc.get_func_attr(
...
class FunctionsTree:
    ...
    def get_list_of_functions(self):
        ...
        seg_ea = idc.get_inf_attr(idc.INF_START_EA)
        functions_list = {}
        for func_ea in idautils.Functions(idc.get_segm_start(seg_ea), idc.get_segm_end(seg_ea)):
            function_name = self.maybe_demangle(idc.get_func_name(func_ea))
    ...
    def build_functions_tree(self, functions_list):
        ...
        for function_name in sorted(functions_list):
            ...
            func_state.flags = idc.get_func_flags(func_state.addr)
...
class FunctionsPlus(PluginForm):
        ...
        for name, tree in sorted(function_tree.items()):
            ...
            if not isinstance(tree, FunctionData):
                ...
                if tree_keys_len % 10 == 1:
                    word = 'item'
                name = f"{name} ({tree_keys_len} {word})"
    ...
    def Show(self, caption, options=0):
        ...
        return PluginForm.Show(self, caption, options=PluginForm.WOPN_PERSIST)
